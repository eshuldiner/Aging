...
# Adaptive Sampling
In comparing the effects of tumor suppressor inactivation in young and old mice, we compared equivalent portions of the tumor size distributions for each sgRNA (i.e., the same number of tumors per infectious unit of virus delivered). To implement this (which we refer to as "adaptive sampling", we scale the number of tumors analyzed for each sgRNA in each cohort to account for differences in viral titer and the number of mice transduced in each cohort, and then analyze the largest tumors per Lenti-sgRNA/Cre vector. The manuscript contains additional details on these statistics and their interpretation; for code related to the implementation of adaptive sampling, see https://github.com/eshuldiner/Eml4-Alk.
